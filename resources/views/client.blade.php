    ...
    <div class=" mx-auto py-7 ">
        <div class="flex items-center justify-center flex-wrap overflow-hidden">
            @foreach ($client as $item)
                @if (auth()->user()->roles->first()->name == 'user' and $item->visibility == 'private')
                ...
                @else
                    <div class="px-4 pt-4 pb-3 m-4 dark:bg-gray-800 bg-white shadow rounded-xl">
                        <a href="{{ $item->url }}" class="text-center">
                            <img src="{{ asset('storage/'.$item->thumbnail) }}" alt="{{ $item->name }}" class="flex sm:w-56 sm:h-28 lg:w-96 lg:h-64 w-72 h-44 min-w-full items-center justify-center dark:bg-gray-700 bg-gray-100">
                            <div class="flex items-center justify-center mt-2">
                                <h1>{{ $item->name }}</h1>
                            </div>
                        </a>
